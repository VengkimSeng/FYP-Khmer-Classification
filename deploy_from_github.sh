...
echo "SSL certificates are NOT included in the GitHub repository for security."
echo "To enable HTTPS, run the SSL upload script separately:"
echo "  ./upload_ssl_certs.sh $SERVER_IP $SSH_USER"
echo ""
echo "Next steps:"
echo "1. Access your application at: http://$SERVER_IP"
echo "2. Upload SSL certificates: ./upload_ssl_certs.sh $SERVER_IP $SSH_USER"
echo "3. Monitor logs: ssh $SSH_USER@$SERVER_IP 'journalctl -u khmer-classifier -f'"
echo "4. To update: ssh $SSH_USER@$SERVER_IP 'cd $APP_DIR && git pull && systemctl restart khmer-classifier'"
echo ""
